...
wordbook = xlsxwriter.Workbook('综艺数据.xlsx')
...
def create_table_header():
    ...
    tv_letter.write(0, 2, 'performer')
    tv_letter.write(0, 3, 'region')
    tv_letter.write(0, 4, 'publish_time')
    tv_letter.write(0, 5, 'imgUrl')
    tv_letter.write(0, 6, 'introduction')
    tv_letter.write(0, 7, 'yearG')
    tv_letter.write(0, 8, 'score')
...
def construct_tv_queue():
    for i in range(0, 2130, 30):
        tv_queue.put(
            "https://v.qq.com/x/bu/pagesheet/list?_all=1&append=1&channel=variety&listpage=2&offset=%s&pagesize=30&sort"
            "=18" % i)
...
def parse_html_object(url):
    ...
    if not re.search("var COVER_INFO = ([\s\S]*)var COLUMN_INFO", response):
        return False
...
def parse_result():
    ...
    global tv_type_count, tv_letter_count, tv_episodes_count, tv_id
    while True:
        ...
        print(object_result['guests'])
        performer = ''
        if object_result['guests'] and len(object_result['guests']) > 0:
            for i in object_result['guests']:
                performer += i + ','
        with lock:
            tv_letter.write(tv_letter_count, 0, tv_id)
            tv_letter.write(tv_letter_count, 1, object_result['title'])
            tv_letter.write(tv_letter_count, 2, performer.rstrip(','))
            print(object_result['area_name'])
            tv_letter.write(tv_letter_count, 3, object_result['area_name'])
            print(url_obj.img_url)
            tv_letter.write(tv_letter_count, 5, url_obj.img_url)
            print(object_result['description'])
            tv_letter.write(tv_letter_count, 6, object_result['description'])
            print(object_result['year'])
            tv_letter.write(tv_letter_count, 7, object_result['year'])
            print(object_result['publish_date'])
            tv_letter.write(tv_letter_count, 4, object_result['publish_date'])
            if object_result['score']:
                print(object_result['score']['score'])
                tv_letter.write(tv_letter_count, 8, object_result['score']['score'])
            ...
            if object_result['nomal_ids'] :
                for i in object_result['nomal_ids']:
                    ...
                    tv_episodes.write(tv_episodes_count, 1, tv_id)
            ...
            if object_result['subtype'] and len(object_result['subtype']) >0:
                for i in object_result['subtype']:
                    tv_type.write(tv_type_count, 0, tv_id)
                    print(i)
